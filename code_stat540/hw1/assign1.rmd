...
There are `r nrow(expDat)` probes and `r ncol(expDat)` samples.
...
`r ncol(nrExpDat)` samples remain. 
...
What is the (estimated) false discovery rate? The estimated FDR is `r q`, by the definition q-value that aims to find the ratio false discoveries out of significant responses.

How many of these hits do we expect to be false discoveries? The expected false discoveries among the hits is `r eFP`.
...
nrow(s1p3KOHits)
```
I find `r nrow(s1p3KOHits)` hits with a difference in expression level between S1P3 knockout and wildtype.
...
nrow(hits)
```
`r nrow(hits)` probes have a BH-adjusted p-value less than 1e-3.
...
```
`r kBr` probes have a BH-adjusted p-value less than 0.1
...
How many probes have a BH-adjusted p-value less than 0.1? `r nrow(hitsGn)` 
