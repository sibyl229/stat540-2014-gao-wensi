...
```{r}
fit <- glmFit(dge.glm.tag.disp, desMat)
lrt <- glmLRT(fit, coef=grep("condition", colnames(desMat)))
edger.results <- topTags(lrt, n=Inf, adjust.method="BH", sort.by="PValue")
```
...
```{r}
edger.results <- edger.results[,c("PValue", "FDR", "logFC", "LR")]
